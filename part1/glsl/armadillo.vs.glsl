...
void main() {
    ...
    */
    ...
    // OriginalArmadilloPosition is the world position of the vertex
    vec3 OriginalArmadilloPosition = (modelMatrix * vec4(position, 1.0)).xyz;
    // Calculate the light direction vector from the vertex to the orb position
    vec3 lightVector = orbPosition - OriginalArmadilloPosition;
    // Normalize the light vector
    vec3 lightDirection = normalize(lightVector); // only direction, not magnitude
    float lightDistance = length(lightVector);

    // Define the attenuation factor
    // Theoretically, the attenuation factor should be 1.0 / (1.0 + distance * distance)
    float attenuation = 1.0 / (1.0 + 0.0005 * lightDistance * lightDistance); 
    //! If we didn't use the attenuation factor, the character will be still light up even if it is far away from the orb.
    ...
    */
    ...
    //! Make sure at the back face, the surface is still light up. (abs)
    ...
    // true means attracting, false means repelling
    ...
    /* Smooth deformation: use smoothstep for smooth transition; float smoothstep(float edge0, float edge1, float x)
    x is the distance from the vertex to the orb
    if x is less than edge0, return 0.0
    if x is greater than edge1, return 1.0
    if x is between edge0 and edge1, return a value between 0.0 and 1.0
    if distanceToOrb = orbRadius, return 1.0
    if distanceToOrb = deformationRange, return 0.0
    */
    ...
    vec3 deformedWorldPosition = deformationFactor > 0.0 ? OriginalArmadilloPosition + (isAttracting ? lightDirection : -lightDirection) * attractionDistance : OriginalArmadilloPosition;
    ...
    // vWorldPosition = OriginalArmadilloPosition;
    //*====================================(c)====================================*/
